...
class NetworkCrystalEntityRenderer(
    ...
    override fun render(
        ...
        if (MinecraftClient.getInstance().player!!.armorItems.any { it.item == ChaoscraftArmors.CRYSTAL_GLASS }) {
            ...
            val vertexConsumer = if (networkCrystalEntity.fetchNetworkDisabled()) {
                vertexConsumerProvider.getBuffer(DISABLED_END_CRYSTAL)
            } else {
                vertexConsumerProvider.getBuffer(END_CRYSTAL)
            }
    ...
    companion object {
        private val TEXTURE = Identifier("textures/entity/end_crystal/end_crystal.png")
        private val NETWORK_DISABLED_TEXTURE = Identifier("textures/entity/end_crystal/end_crystal_disabled.png")

        val END_CRYSTAL: RenderLayer = RenderLayer.getEntityCutoutNoCull(TEXTURE)
        val DISABLED_END_CRYSTAL: RenderLayer = RenderLayer.getEntityCutoutNoCull(NETWORK_DISABLED_TEXTURE)
